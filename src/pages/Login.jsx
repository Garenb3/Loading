...
export default function Login() {
  ...
  const handleLogin = e => {
    ...
    if (email === "test@test.com" && password === "1234") alert("Login successful");
    ...
    >
      ...
      <div className="flex justify-center items-center py-12">
        <form onSubmit={handleLogin} className="bg-secondary p-6 rounded-lg shadow-md w-80" style={{ backgroundColor:"var(--secondary)", color:"var(--text)" }}>
          ...
          <button className="w-full p-2 rounded" style={{ backgroundColor:"var(--primary)", color:"#fff" }}>Login</button>
        </form>
      </div>
    </div>
  )
